...
- 用户已于 2026-08-06 明确授权：每次任务产生文件改动并完成验证后，提交本任务相关文件并 push 当前分支，不再默认把改动只留在本地。
- 提交前必须检查当前分支、远端同步状态和完整工作区，避免混入其他 agent 的未审计改动、数据、checkpoint、缓存、secret 或生成式大文件。
- 只暂存经过审计且属于本次提交范围的具体路径；提交后执行普通 push。若远端前进、权限失败、hook/测试失败或存在冲突，禁止 force push、禁止跳过 hook，保留本地改动并向用户报告。
- 没有文件改动时不创建空提交；已冻结历史输出和正式结果仍不得为了提交而覆盖或改写。
