#!/usr/bin/env python3
"""Replicate frozen S19 fusion with structural high-SNR B1 fallback."""
...
STAGES = ("b0", "diffusion", "b1", "control", "fusion", "routed_control", "routed_fusion")


def validate(config: dict[str, Any]) -> None:
    protocol = config["protocol"]
    if protocol["status"] != "preregistered_before_cross_population_replication_output":
        raise RuntimeError("S26 config is not executable")
    for key in (
        "s19_selection_and_holdout_outcomes_known",
        "s25_negative_outcome_known",
        "target_population_b1_s23_outcomes_known",
        "target_population_s19_checkpoint_outputs_unknown",
        "frozen_s19_checkpoints_no_retraining",
        "no_target_selection_or_tuning",
        "one_shot_target_holdout_evaluation",
    ):
        if protocol.get(key) is not True:
            raise RuntimeError(f"S26 protocol boundary changed: {key}")
    ...
    for key, hash_key in (
        ...
        if not path.is_file() or sha256_file(path) != str(config["inputs"][hash_key]):
            raise RuntimeError(f"S26 frozen input hash mismatch: {key}")
    route = config["exact_fallback"]
    if [float(value) for value in route["use_frozen_branch_snrs_db"]] != [1.0, 4.0, 7.0]:
        raise RuntimeError("low-SNR route changed")
    if [float(value) for value in route["use_exact_b1_snrs_db"]] != [13.0, 19.0]:
        raise RuntimeError("exact fallback route changed")
    if int(route["learned_parameters"]) != 0 or int(route["side_information_real_symbols"]) != 0:
        raise RuntimeError("S26 route introduced parameters or side information")
    if int(config["rate"]["active_real_symbols"]) != 19712:
        raise RuntimeError("strict rate contract changed")


def majority_correct(row: dict[str, Any], stage: str) -> bool:
    return sum(
        int(row[f"{name}_{stage}_prediction"]) == int(row[f"{name}_original_prediction"])
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for stage in STAGES:
        for metric in ("psnr", "ms_ssim", "lpips"):
            result[f"mean_{stage}_{metric}"] = float(
                np.mean([float(row[f"{stage}_{metric}"]) for row in rows])
            )
        correct = [majority_correct(row, stage) for row in rows]
        anchor = [majority_correct(row, "b1") for row in rows]
        result[f"majority_{stage}_failure"] = sum(not value for value in correct)
        result[f"majority_{stage}_new_vs_b1"] = sum(
            b1_value and not stage_value for b1_value, stage_value in zip(anchor, correct)
        )
        result[f"majority_{stage}_repair_vs_b1"] = sum(
            not b1_value and stage_value for b1_value, stage_value in zip(anchor, correct)
        )
    for left, right in (
        ("routed_fusion", "routed_control"),
        ("routed_fusion", "b1"),
        ("routed_control", "b1"),
    ):
        for metric in ("psnr", "ms_ssim", "lpips"):
            result[f"{left}_minus_{right}_{metric}"] = (
                result[f"mean_{left}_{metric}"] - result[f"mean_{right}_{metric}"]
            )
    result["per_snr"] = []
    for snr in result["snrs_db"]:
        ...
        for left, right in (("routed_fusion", "routed_control"), ("routed_fusion", "b1")):
            for metric in ("psnr", "lpips"):
                item[f"{left}_minus_{right}_{metric}"] = (
                    item[f"mean_{left}_{metric}"] - item[f"mean_{right}_{metric}"]
                )
        item["max_routed_fusion_b1_difference"] = max(
            float(row["routed_fusion_b1_difference"]) for row in subset
        )
        item["max_routed_control_b1_difference"] = max(
            float(row["routed_control_b1_difference"]) for row in subset
        )
        result["per_snr"].append(item)
    return result


def bootstrap(
    rows: list[dict[str, Any]], left: str, right: str, metric: str, seed: int, replicates: int
) -> dict[str, Any]:
    grouped: defaultdict[str, list[float]] = defaultdict(list)
    for row in rows:
        if metric == "majority_failure":
            value = float(not majority_correct(row, left)) - float(not majority_correct(row, right))
        else:
            value = float(row[f"{left}_{metric}"]) - float(row[f"{right}_{metric}"])
        grouped[str(row["sample"])].append(value)
    names = sorted(grouped)
    matrix = np.asarray([grouped[name] for name in names], dtype=np.float64)
    if matrix.ndim != 2 or matrix.shape[1] != 5:
        raise RuntimeError(f"unexpected cluster matrix: {matrix.shape}")
    cluster_values = matrix.mean(axis=1)
    rng = np.random.default_rng(seed)
    draws = np.empty(replicates, dtype=np.float64)
    for index in range(replicates):
        selected = rng.integers(0, len(names), size=len(names))
        draws[index] = float(cluster_values[selected].mean())
    return {
        "left": left,
        "right": right,
        "metric": metric,
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s26_s19_exact_fallback_replication.yaml")
    ...
    for batch_index, batch in enumerate(loader):
        ...
        for name, classifier in classifiers.items():
            ...
            for stage, image in candidates.items():
                predictions[name][stage], confidences[name][stage] = classify(
                    classifier, image, mean, std
                )
        for index, sample in enumerate(batch["sample"]):
            row: dict[str, Any] = {
                ...
                "routed_fusion_b1_difference": float(
                    (routed_fusion[index] - b1_output[index]).abs().max().cpu()
                ),
                "routed_control_b1_difference": float(
                    (routed_control[index] - b1_output[index]).abs().max().cpu()
                ),
            }
            for stage in STAGES:
                for metric in ("psnr", "ms_ssim", "lpips"):
    ...
    if len(rows) != 1280:
        raise RuntimeError(f"S26 row count changed: {len(rows)}")
    ...
    summary.update(
        {
            "analysis_id": config["analysis_id"],
            "claim_scope": "frozen_cross_population_replication_on_previously_exposed_image_population",
            ...
            "holdout_role_accessed_once": True,
    ...
    save_json(
        output / "STATE.json",
        {"state": "CROSS_POPULATION_REPLICATION_COMPLETE", "summary_sha256": sha256_file(summary_path), **summary},
    )
